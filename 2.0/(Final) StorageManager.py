...
ftp_host = '127.0.0.1'
ftp_user = input('User ID: ')
ftp_passwd = input('User Password: ')
...
def identify_virus_patterns(folder):
    virus_pattern = re.compile(r'\b[A-Za-z]{2}\d{6}\b') #boundary anchor, matches the empty string at the beg/end of a word
    virus_files = []
...
def create_encrypted_zip(files, archive_name, password, output_folder):
    ...
    with pyzipper.AESZipFile(zip_path, 'w', compression=pyzipper.ZIP_LZMA) as zipf:
        # Set the password for the ZIP file, encoding it in UTF-8.
        zipf.setpassword(password.encode('utf-8'))
        # Set the encryption method to AES 256-bit.
        zipf.setencryption(pyzipper.WZ_AES, nbits=256)
        for file in files:
            zipf.write(file, os.path.basename(file))
            # Log the addition of the file to a summary log.
            log_to_summary(file, archive_name)
    logging.info(f'Archive created: {archive_name}.zip with encryption.')

# Function to format a date into a specific format (YYYYMM).
...
ftp_user = 'admin'
ftp_passwd = 'admin'
remote_folder = '/resource/serverClean'
local_zip_folder = os.path.join(os.getcwd(), 'clientClean')
...
password = input('Decrypting password: ')
